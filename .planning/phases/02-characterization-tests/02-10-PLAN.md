...
must_haves:
  truths:
    - "Relayer validator has characterization tests covering request validation logic"
    - "Relayer middleware has characterization tests covering request/response processing"
    - "Relayer relay meter has characterization tests covering stake metering"
...
<task type="auto">
  ...
  <action>
    ...
    **Pattern:** Use `httptest.NewRecorder()` and `http.NewRequest()` for testing middleware in isolation. Use local mock types consistent with proxy_test.go patterns.
  </action>
  <verify>
    ```bash
    # Tests compile
...
<task type="auto">
  ...
  <action>
    ...
    **Pattern:** Use suite-based tests with miniredis. Create helper functions for setting up meter with mock dependencies. Use local mock types for cache and session lookups.
  ...
  <done>
    relay_meter_test.go has 8+ tests. All pass with -race and -shuffle=on. Relayer coverage has increased from 6.8% baseline.
  </done>
</task>
