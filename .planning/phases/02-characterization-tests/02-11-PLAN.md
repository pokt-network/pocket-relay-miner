...
must_haves:
  truths:
    - "Cache package has characterization tests covering L1/L2/L3 fallback pattern"
    - "Cache interface helpers (CacheKeys, CacheConfig) have unit tests"
    - "SharedParamsSingleton cache has tests for Get, Set, Invalidate, and distributed locking"
    - "ApplicationCache has tests for keyed entity cache L1/L2/L3 pattern"
...
<task type="auto">
  ...
  <action>
    ...
    **Important:** The cache constructors may require specific dependencies. Read the constructor signature and provide mock implementations for all required dependencies. Use miniredis for Redis, and mock interfaces for chain queries.
  </action>
  <verify>
    ```bash
    # Tests compile
...
<task type="auto">
  ...
  <action>
    ...
    **Important:** Use `miniredis.FastForward(duration)` to simulate time passing for TTL tests (deterministic, no time.Sleep). Use proto.Marshal/Unmarshal for test data if the cache stores protobuf-encoded data.
  </action>
  <verify>
    ```bash
    # Tests compile
  ...
  <done>
    application_cache_test.go has 10+ tests. All pass with -race and -shuffle=on. Cache coverage has increased from 0% baseline.
  </done>
</task>
...
4. Cache coverage increased from 0% baseline to measurable level
...
- Cache/ coverage meaningfully above 0%
</success_criteria>
